...
class UserRepository extends BaseRepository
{
    ...
    public function store(array $input)
    {
        ...
        $doctorArray = Arr::only($input, ['first_name','no_str','experience', 'twitter_url', 'linkedin_url', 'instagram_url', 'profile']);
        ...
        try {
            ...
            if (isset($input['profile_image']) && ! empty('profile_image')) {
                $doctor->addMedia($input['profile_image'])->toMediaCollection(User::PROFILE, config('app.media_disc'));
            }
    ...
    public function update($input, $doctor)
    {
        ...
        $doctorArray = Arr::only($input, ['first_name', 'clinic_id', 'no_str','experience', 'twitter_url', 'linkedin_url', 'instagram_url', 'profile']);
        ...
        try {
            ...
            if (isset($input['profile_image']) && ! empty('profile_image')) {
                ...
                $doctor->user->addMedia($input['profile_image'])->toMediaCollection(User::PROFILE, config('app.media_disc'));
            }
            DB::commit();
    ...
    public function updateProfile(array $userInput): bool
    {
        try {
            ...
            if ((getLogInUser()->hasRole('patient'))) {
                if (! empty($userInput['profile_image'])) {
                    $user->clearMediaCollection(Patient::PROFILE);
                    $user->patient->media()->delete();
                    $user->patient->addMedia($userInput['profile_image'])->toMediaCollection(Patient::PROFILE,
                ...
                if ((! empty($userInput['profile_image']))) {
                    ...
                    $user->media()->delete();
                    $user->addMedia($userInput['profile_image'])->toMediaCollection(User::PROFILE, config('app.media_disc'));
                }
            }
    ...
    public function prepareData($doctor): array
    {
        $data2['clinic'] = User::where('type',4)->orderBy('first_name', 'ASC')->pluck('first_name', 'id');
