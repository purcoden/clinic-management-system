...
@section('header_toolbar')
    <div class="container-fluid">
        <div class="d-flex flex-wrap align-items-center justify-content-between mb-7">
            <h1 class="mb-0 me-1">{{__('messages.clinic.clinic_details')}}</h1>
                        ...
                        <div class="col-xxl-5 col-12">
                            <div class="d-sm-flex align-items-center mb-5 mb-xxl-0 text-center text-sm-start">
                                <div class="image image-circle image-lg-small">
                                    <img src="{{ $clinic->profile_image }}" alt="user">
                                </div>
                                <div class="ms-0 ms-md-10 mt-5 mt-sm-0  ">
                                    <span class="text-success mb-2 d-block">{{ $clinic->role_name }}</span>
                                    <h2>{{ $clinic->name }}</h2>
                                    <a href="mailto:{{ $clinic->email }}"
                                       class="text-gray-600 text-decoration-none fs-4">
                                        {{ $clinic->email }}
                                    </a><br>
                                    <a href="tel:{{ $clinic->region_code }} {{ $clinic->contact }}"
                                       class="text-gray-600 text-decoration-none fs-4">
    {{ !empty($clinic->contact) ? '+'. $clinic->region_code .' '. $clinic->contact  : __('messages.common.n/a') }}
                                        </a>
                                    </div>
                                </div>
                            </div>
