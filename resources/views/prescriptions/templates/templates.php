...
<script id="prescriptionMedicineTemplate" type="text/x-jsrender">
    <tr>
        <td class="table__item-desc">
            <select class="form-select prescriptionMedicineId quantityget select2Selector" name="medicine[]" placeholder="select medicine" data-id="{{:uniqueId}}" required>
        ...
        <td>
            <input class="form-control prescription-dose" required="" name="dosage[]" type="text">
            <small class=" text-danger avalable-amount"></small>
        </td>
        <td>


            <select class="form-select DoseDurationIdTemplate select2Selector" name="day[]" placeholder="select dose Interval" 'data-control' => 'select2' data-id="{{:uniqueId}}" required>
                {{for doseDuration}}
                    <option value="{{:key}}">{{:value}}</option>
                {{/for}}
            </select>
        ...
        <td>
            <div class="extrm">
                <select class="form-select prescriptionMedicineMealId" name="time[]" data-id="{{:uniqueId}}" 'data-control' => 'select2'>
                    {{for meals}}
                        <option value="{{:key}}">{{:value}}</option>
                    {{/for}}
                </select>
                <div class="d-flex">
                    <small class="text-danger totalqty extra-margin d-none" id="totalqty{{:uniqueId}}">Total Quentity :</small>
                    <small class="text-danger quantityshow" id="quantityshow{{:uniqueId}}"></small>
                </div>
            </div>
    ...
    <!-- <td>
    <div class="d-flex">
            <span class="float-left text-danger totalqty d-none">Total Quentity :</span>
            <span class="text-danger quantityshow" id="quantityshow{{:uniqueId}}"></span>
        </div>
    </td> -->
