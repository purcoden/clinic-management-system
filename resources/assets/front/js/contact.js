			...
			var contactForm = function()	{
				...
				window.expiredRecaptchaCallback = function () {
				 ...
				 jQuery('.ajax-form').on('submit', function( event ) {
					 ...
					 if ($('#error-msg').text() !== '') {
						 $('#phoneNumber').focus();
						 var response = '<div class="err alert alert-danger">Contact number is '  + $("#error-msg").text() +'</div>';
						 $('.ajax-form .ajax-message').html(response).delay(5000).hide('slow');
						 return false;
					 }
						var grecaptchaResponse = grecaptcha.getResponse();
						if (grecaptchaResponse.length == 0) {
							var response =	'<div class="alert alert-danger alert-dismissable">'+ Lang.get('messages.common.google_captcha_required') +'</div>';
						...
						jQuery.ajax({
							...
							{
								if(data.success){
									response = '<div class="gen alert alert-success">'+Lang.get('messages.flash.messages_sent')+'</div>';
							...
							error : function(data){
								response = '<div class="err alert alert-danger">'+data.responseJSON.message+'</div>';
								$('.ajax-form .ajax-message').html(response).delay(5000).hide('slow');
							},
